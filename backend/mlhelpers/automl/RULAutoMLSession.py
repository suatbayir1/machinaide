...
import pandas as pd
from ml_helpers import data_status, return_default_value, return_operator_info
...
class RULAutoMLSession:
    def __init__(self, settings) -> None:
        ...
        if(("windowLength" in settings) and len(settings["windowLength"])):
    ...
    def start_tuning(self, seq_array, label_array, seq_len, features, train_df):
        ...
        settings = {
            ...
            # "numberOfFeatures": self.number_of_features,
    ...
    def prepare_data(self):
        # TODO: get failures 
        failure_res = requests.post(url=GET_FAILURES_URL, json={"sourceName": "Press031"}, headers={'token': self.token, 'Content-Type': 'application/json'}).json()
        print(failure_res)
        """ failure_res = [
            {
        ...
        continue_process = data_status(self.type, failures, self.experiment_name, self.database, self.measurement, self.field, self.query_helper, self.product_id, self.start_time, self.creator, self.min_data_points, "rul")
        ...
        if(continue_process):
            ...
            for failure in failures:
                ...
                if(self.type == "machine"):
                    ...
                    measurements = self.get_query_results("measurement", self.database)
                    print("measurements: ", measurements)
                    all_data = []
                    # TODO: delete next line it was for testing
                    ...
                    for measurement in measurements:
                        ...
                        fields = self.get_query_results("field", self.database, measurement)
                        print("fields: ", fields)
                        ...
                        for field in fields:
                            # print(field)
                            # TODO: do we have null value replacer?
                            # sensor_info = requests.get(url=GETSENSORFROMMAPPING + field).json()
                            # TODO: do fillnan operation
                            ...
                            data = self.get_query_results("sensor", self.database, measurement, field, duration_start_date, duration_end_date)
                            ...
                            if(not df.empty):
                                ...
                                if(do_fill_nan):
                                    field_name = f"{measurement}.{field}"
                    ...
                    fields = self.get_query_results("field", self.database, self.measurement)
                    print("fields: ", fields)
                    # remove sensor that are selected as not used in ml from fields
                    for not_used_sensor in not_used_sensors:
                        if(not_used_sensor in fields):
                    ...
                    for field in fields:
                        ...
                        if(sensor_info):
                            ...
                            if("operator" and "operatorValue" in sensor_info):
                                ...
                                operator, operator_value = (None, None)
                        data = self.get_query_results("sensor", self.database, self.measurement, field, duration_start_date, duration_end_date)
                        ...
                        if(not df.empty):
                            ...
                            # do fillnan operation if user selected a default value for field
                            if(do_fill_nan):
                                field_name = f"{self.measurement}.{field}"
                                df = filler.get_df_with_values(df, field_name, default_value).dropna()
                    ...
                    data = self.get_query_results("sensor", self.database, self.measurement, self.field, duration_start_date, duration_end_date)
                    df = pd.DataFrame(data)
                    # df.fillna(-666).to_csv("./sensor-null-data.csv") 
                    data_points = []
                    if(not df.empty):
                        filler = FillNanValues(operator, operator_value, is_numeric, default_value)
                        # do fillnan operation if user selected a default value for field
                        if(do_fill_nan):
                            field_name = f"{self.measurement}.{self.field}"
                            df = filler.get_df_with_values(df, field_name, default_value).dropna()
                    ...
                    for point in data_points:
                        point["time"] = cycle
                        cycled_points.append(point)
                        cycle += 1

                    one_fail_data = one_fail_data + cycled_points  
