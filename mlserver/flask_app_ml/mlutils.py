...
def to_sequences(dataset, input_columns, output_columns, input_dim, output_dim, in_seq_size, out_seq_size, in_means, in_stds, out_means, out_stds):
    ...
    return np.array(x).reshape(len(x), in_seq_size, input_dim),np.array(y).reshape(len(y), output_dim)
...
class QueryHelper:
    def __init__(self, settings):
        ...
        self._alertURL = 'http://localhost:7494/anomaly'
    ...
    def query(self, measurement_to_sensors, startTime=None, endTime=None):
        if startTime is not None and endTime is not None:
            ...
            queries = self._build_influx_queries(measurement_to_sensors, "now() - 1m", "now()")
            print(queries)
        try:
