...
import {
    ...
    AlignItems, InputToggleType, Toggle, FlexBox, Tabs, GradientBox
...
class EvidentlyReportPage extends PureComponent<Props, State>{
    constructor(props) {
        super(props)
        this.state = {
    ...
    async componentDidMount() {
        await HealthAssessmentService.getEvidentlyReport(this.props.match.params.MID).then((report)=>{
            ...
            HealthAssessmentService.getMLModel({"modelID": this.props.match.params.MID, "task": report["task"]}).then(model=>{
                ...
                this.setState({model: model, report: report["report"], modelTask: report["task"]})
            })
        })
...
export default EvidentlyReportPage