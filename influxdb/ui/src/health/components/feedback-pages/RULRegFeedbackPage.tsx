...
import RULRegFeedbackBrushGraph from 'src/health/components/feedback-pages/RULRegFeedbackBrushGraph'
...
class RULRegFeedbackPage extends PureComponent<Props, State>{
    state = {
        timeRange: {
            seconds: 3600,
            lower: "now() - 1h",
    ...
    getMLModels = async () => {
        ...
        const models = await HealthAssessmentService.getMLModelsWithID({"pipelineID": modelID})
    ...
    */
    ...
    render(): React.ReactNode {
        return(
            <Page
                titleTag="Remaining Useful Lifetime Regression Model Feedback"
                className="alert-history-page"
            >
                <Page.Header fullWidth={true}>
                    <Page.Title
                        title="Remaining Useful Lifetime Regression Model Feedback"
                    />
                    <div className="tabbed-page--header-right">
                        <Button
                            ...
                            onClick={() => this.props["history"].push(`/orgs/${this.props["match"]["params"]["orgID"]}/health/regression-report/${this.props.match.params["MODELID"]}`)}
                        />
                    </div>
                </Page.Header>
                            ...
                            <Grid.Column widthXS={Columns.Twelve} style={{height: "700px"}}>
                                <Panel style={{border: 'solid 2px #999dab', borderRadius: '4px', height: "inherit"}}>
                                    ...
                                    <Panel.Body size={ComponentSize.Small}>
                                        <div>
                                            {this.state.isDataLoaded ? <RULRegFeedbackBrushGraph  /* this.state.machineSensorData.length */
...
export default RULRegFeedbackPage