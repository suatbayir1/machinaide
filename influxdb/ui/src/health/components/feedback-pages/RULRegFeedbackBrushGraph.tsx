...
interface State {
    ...
    filterRULRegCycles: string
}

//const lineChartRef = useRef(null)
//const brushChartRef = useRef(null)

class RULRegFeedbackBrushGraph extends Component<Props, State> {
    constructor(props){
        super(props)
        // console.log(props)
    ...
    filterModelLogs = (addAnnotation=false) => {
      ...
      let filteredModelLogs : object[] = []
      if(allModelLogs){
        for(let log of allModelLogs){
          let remaningCycles = log["prediction"]
          if(remaningCycles>=0 && remaningCycles <= parseInt(this.state.filterRULRegCycles)){
    ...
    getModelLogsData = async () => {
      ...
      if(model){
        if(("pipelineID" in model) ){
            AutoMLService.getModelLogsOnContion(model["pipelineID"], model["task"]).then(res=>{
                //console.log(res)
    ...
    getInRangeFailures = (log) => {
      if(log){
        ...
        console.log("inRangeFailures", inRangeFailures, log)
                            ...
                            <Grid>
                                ...
                                <Grid.Row>
                                  ...
                                  <Grid.Column widthXS={Columns.Four}>
                                    <div className="tabbed-page--header-left">
                                      <Input
                                          onChange={(e) => this.setState({ filterRULRegCycles: e.target.value })}
                                          name="filterRULRegCycles"
                                          type={InputType.Text}
                                          value={this.state.filterRULRegCycles}
                                          placeholder={"Enter remaining cycles"}
                                          style={{width: "60%"}}
                                      />
                                    </div>
                                  </Grid.Column>
                            ...
                            {this.state.showModelLogs ? (
                                ...
                                <Table.Header>
                                    <Table.Row>
                                        ...
                                        <Table.HeaderCell style={{width: "70px"}}>Cycles Left</Table.HeaderCell>
...
export default RULRegFeedbackBrushGraph