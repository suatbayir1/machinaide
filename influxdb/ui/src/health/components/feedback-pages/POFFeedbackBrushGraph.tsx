...
interface State {
    ...
    filterPOFCycles: string
    filterPOFProbability: string
}

//const lineChartRef = useRef(null)
//const brushChartRef = useRef(null)

class POFFeedbackBrushGraph extends Component<Props, State> {
    constructor(props){
        super(props)
        // console.log(props)
        let stop = new Date().toISOString()
        let start = new Date(new Date().getTime() - 24*60*60*365*1000).toISOString()
        let start30 = new Date(new Date().getTime() - 24*60*60*7*1000).toISOString()
        this.state = {
            filterTimeRange: {
              lower: start,
              upper: stop,
    ...
    handleChooseTimeRange = (timeRange: TimeRange) => {
      this.setState({ filterTimeRange: timeRange }, () => this.filterAllAndAnnotate("failure"))
      console.log(timeRange)
    }

    handleChooseTimeRangeLogs = (timeRange: TimeRange) => {
      this.setState({filterTimeRangeLogs: timeRange}, () => this.filterAllAndAnnotate("logs"))
      console.log(timeRange)
    }
    ...
    onSelectFilterSeverity = (option: string) => {
      ...
      this.setState({ filterSeverity: updatedOptions }, ()=>this.filterAllAndAnnotate("failure"))
    }

    filterFailures = (addAnnotation=false) => {
      ...
      if(allFailures){
        ...
        if(addAnnotation)
          this.setState({shownRelatedFailures: filteredFailures}, () => this.addAnnotationsOnGraph())
        else
          this.setState({shownRelatedFailures: filteredFailures})
      }
    }

    filterModelLogs = (addAnnotation=false) => {
      let allModelLogs = this.state.modelsLogsData
      let filteredModelLogs = []
      if(allModelLogs){
        for(let log of allModelLogs){
          let prob = this.findProbability(log["prediction"][0], log["prediction"][1], parseInt(this.state.filterPOFCycles))
          if(prob >= parseFloat(this.state.filterPOFProbability)){
        ...
        if(addAnnotation)
          this.setState({shownModelsLogsData: filteredModelLogs}, () => this.addAnnotationsOnGraph())
        else
    ...
    addAnnotationsOnGraph = () => {
      let failures = this.state.shownRelatedFailures
      let modelsLogs = this.state.shownModelsLogsData
      let annotations = []
      let lineAnnotations = []
      if(failures){
          ...
          if(log["time"] && startTimeDate instanceof Date && !isNaN(startTimeDate.valueOf())){
            let anno = {
              ...
              label: {
                ...
                text: `Model log: ${this.findProbability(log["prediction"][0], log["prediction"][1])}`
              }
            }
    ...
    filterAllAndAnnotate = (filterType="all") => {
      if(filterType === "failure"){
        this.filterFailures(true)
      }
      else if(filterType === "modellog"){
        this.filterModelLogs(true)
      }
      else{
        this.filterFailures()
        this.filterModelLogs(true)
    ...
    getModelLogsData = async () => {
      let model = this.props.model
      if(model){
        if(!("pipelineID" in model) && ("modelID" in model)){
            AutoMLService.getModelLogsOnContion(model["modelID"], model["task"]).then(res=>{
                //console.log(res)
                if(model["task"] === "pof"){
                    this.setState({modelsLogsData: res, shownModelsLogsData: res}, ()=>this.filterAllAndAnnotate()) // , () => this.addAnnotationsOnGraph()
                }  
                            ...
                            <Grid>
                                ...
                                <Grid.Row>
                                  ...
                                  <Grid.Column widthXS={Columns.Four}>
                                    <div className="tabbed-page--header-left">
                                      <Input
                                          onChange={(e) => this.setState({ filterPOFProbability: e.target.value })}
                                          name="filterPOFProbability"
                                          type={InputType.Text}
                                          value={this.state.filterPOFProbability}
                                          placeholder={"Enter cycles number"}
                                          style={{width: "60%"}}
                                      />
                                    </div>
                                  </Grid.Column>
                                ...
                                <Grid.Row>
                                  ...
                                  <Grid.Column widthXS={Columns.Four}>
                                    <div className="tabbed-page--header-left">
                                      <Input
                                          onChange={(e) => this.setState({ filterPOFCycles: e.target.value })}
                                          name="filterPOFCycles"
                                          type={InputType.Text}
                                          value={this.state.filterPOFCycles}
                                          placeholder={"Enter probability number"}
                                          style={{width: "650%"}}
                                      />
                                    </div>
                                  </Grid.Column>
                                  ...
                                  <Grid.Column widthXS={Columns.Three}>
                                    <div className="tabbed-page--header-left">
                                      <SquareButton
                                        ...
                                        onClick={() => this.filterAllAndAnnotate("modellogs")} 
                                        titleText={"Filter model logs"}
                                        //status={status}
                            ...
                            {this.state.showModelLogs ? (
                                ...
                                <Table.Header>
                                    <Table.Row>
                                        ...
                                        <Table.HeaderCell style={{width: "70px"}}>Probability</Table.HeaderCell>
                                    ...
                                    {this.state.shownModelsLogsData.map((log, i) => {
                                        ...
                                        <Table.Row key={uuid.v4()}>
                                            ...
                                            <Table.Cell style={{width: "70px"}}>
                                                {this.findProbability(log["prediction"][0], log["prediction"][1])}
                                            </Table.Cell>
...
export default POFFeedbackBrushGraph