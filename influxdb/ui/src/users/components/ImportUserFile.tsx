...
class ImportUserFile extends PureComponent<Props, State> {
        ...
        result.forEach(async (row, index) => {
            ...
            if (!isErrorLine) {
                if (row["username"] !== "" && row["password"] !== "") {
                    ...
                    let response = await UserService.isUserAlreadyExist(payload);

                    if (response.data.summary.code === 409) {
                        let error = {
                            "type": "ERROR",
                            "color": "red",
                            "text": `In line: ${index + 1} ERROR: This username already exists and username must be unique (${row["username"]})`
                        }
    ...
    private checkAreThereAnyError = () => {
        if (this.state.errors.length > 1) {
            this.state.errors.forEach((row, index) => {
                if (row["type"] === "INFO") {
                    this.setState({
            ...
            if (isErrorLine) {
                ...
                if (row["username"] !== "" && row["password"] !== "") {
                    ...
                    if (updateResult === 204) {
                        ...
                        await UserService.addUserToMongo(addUserToMongoPayload);
                        this.props.getUsers();
                    }
                }
            }
