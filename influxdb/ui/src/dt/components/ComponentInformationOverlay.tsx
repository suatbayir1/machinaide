...
import {
    ...
    Input, TextArea, FlexBox, FlexDirection, Overlay
...
type Props = {
    ...
    objectList: object[]
    isComponentInformationOverlayVisible: boolean
    handleDismissComponentInformationOverlay: () => void
}
...
class ComponentInformationOverlay extends PureComponent<Props, State> {
    constructor(props) {
        super(props);
    ...
    private updateComponent = async (): Promise<void> => {
        ...
        if (updatedResult.summary.code === 200) {
            this.props.handleDismissComponentInformationOverlay()
    ...
    public render() {
        const { selectedGraphNode, clickPartDetail, clickBrands, objectList, isComponentInformationOverlayVisible, handleDismissComponentInformationOverlay } = this.props;
        const { displayName, description, visibleConfirmationOverlay, selectedObject } = this.state;

        return (
            <>
                <Overlay visible={isComponentInformationOverlayVisible}>
                    <Overlay.Container maxWidth={750}>
                        <Overlay.Header
                            title="Edit Component"
                            onDismiss={handleDismissComponentInformationOverlay}
                        ...
                        <Overlay.Body>
                            ...
                            <Form key={selectedGraphNode["id"]}>
                                <Grid>                                    
                                    ...
                                    <Grid.Row>
                                        <Grid.Column widthXS={Columns.Twelve}>
                                            <Form.Element label={`Sensor List (${selectedGraphNode["sensors"] ? selectedGraphNode["sensors"].length : 0})`}>
                                                {
                                                    selectedGraphNode["sensors"] && selectedGraphNode["sensors"].length > 0 ?
                                                        <DapperScrollbars
                                                            autoHide={false}
                                                            autoSizeHeight={true}
...
export default ComponentInformationOverlay;